...
if [ -f "$instalador_remoto" ]; then
    ZOOM_VERSION_AVAILABLE=$(dpkg-deb -I $instalador_remoto | grep Version | awk '{print $2}')
    ...
    ZOOM_VERSION_AVAILABLE=$(dpkg-deb -I "$instalador_local" | grep Version | awk '{print $2}')
    # echo "ultima versão disponível para download: $ZOOM_VERSION_AVAILABLE"
fi
