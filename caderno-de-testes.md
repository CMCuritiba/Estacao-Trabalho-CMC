...
| ID  | Teste | Como testar |
|-----|-------|-------------|
| 021 | Programas removidos | Verificar se programas desnecessários foram removidos<br>Comando: `dpkg -l | grep -E "(hexchat|thunderbird|hypnotix|drawing|celluloid|pix|webapp-manager|warpinator|sticky|simple-scan|transmission)"`<br>Não deve retornar nenhum resultado |
...
| ID  | Teste | Como testar |
|-----|-------|-------------|
| 061 | Políticas do Chrome | Verificar arquivo de políticas do Chrome<br>Comando: `cat /etc/opt/chrome/policies/managed/policies.json`<br>Deve conter configurações como bookmarks gerenciados |
...
| ID  | Teste | Como testar |
|-----|-------|-------------|
| 081 | Arquivo política criado | Verificar se arquivo de política foi criado<br>Comando: `ls -la /home/*/Desktop/politica-informatica.pdf`<br>Arquivo deve existir no desktop |
...
| ID  | Teste | Como testar |
|-----|-------|-------------|
| 101 | Configurações dconf aplicadas | Verificar configurações do ambiente gráfico<br>Comando: `dconf dump /` \| grep -i cinnamon<br>Verificar se configurações personalizadas estão aplicadas |
...
| ID  | Teste | Como testar |
|-----|-------|-------------|
| 111 | PolicyKit configurado | Verificar arquivo de política<br>Comando: `cat /etc/polkit-1/localauthority/50-local.d/10-terminal.pkla`<br>Deve conter regras de acesso ao terminal |
...
| ID  | Teste | Como testar |
|-----|-------|-------------|
| 131 | Script boot criado | Verificar se script existe<br>Comando: `ls -la /opt/cmc/cmc-boot.sh`<br>Script deve existir e ser executável |
...
| ID  | Teste | Como testar |
|-----|-------|-------------|
| 272 | ACLs configuradas | Verificar ACLs nos executáveis<br>Comando: `getfacl /usr/bin/cinnamon-settings`<br>Verificar permissões |
...
| ID  | Teste | Como testar |
|-----|-------|-------------|
| 282 | Script on-login criado | Verificar se script existe<br>Comando: `ls -la /opt/cmc/on-login.sh`<br>Script deve existir |
...
**Total de testes manuais: 40+ (distribuídos conforme numeração das tasks)**
